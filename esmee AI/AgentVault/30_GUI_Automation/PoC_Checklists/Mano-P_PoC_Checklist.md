...
- `runtime_gate_review_completed_waiting_help_only_candidate_package_confirmation`
...
- 已完成安装计划、执行清单、元数据查询、formula/dependency 审查、Python source 审查、runtime gate plan 和部门复审；本轮未克隆、未安装、未运行、未下载模型、未授予权限。
- 工程部和安全部均已只读审查，结论为 `approved_with_notes`。
- 下一步如继续，只允许准备 help-only runtime 候选包；仍不允许直接运行 `mano-cua`。
